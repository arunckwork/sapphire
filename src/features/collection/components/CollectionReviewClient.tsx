        ...
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 px-3 py-2 rounded-2xl bg-white/10 backdrop-blur-sm max-w-[90vw] overflow-x-auto">
          {urls.map((u, i) => (
            <button
              ...
              className={`shrink-0 h-10 w-10 rounded-md overflow-hidden border-2 transition-all ${i === index ? 'border-amber-400 scale-110' : 'border-transparent opacity-60 hover:opacity-100'
                }`}
            >
              {/* eslint-disable-next-line @next/next/no-img-element */}
...
export function CollectionReviewClient({ id, isFromInventory = false }: CollectionReviewClientProps) {
  ...
  const inputCls = (hasErr: boolean) =>
    `w-full rounded-lg border px-3.5 py-2.5 text-sm bg-background text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-amber-500/50 transition-colors ${hasErr ? 'border-rose-500/60' : 'border-border'
    }`;
    ...
    <>
      <div className="space-y-6 max-w-4xl mx-auto">
        ...
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          <Link href={isFromInventory ? "/inventory" : "/collection"} className="hover:text-amber-600 transition-colors">{isFromInventory ? "Inventory" : "Collections"}</Link>
          <span>/</span>
        ...
        {isAccepted && (
          <div className="flex items-center justify-between">
            <Link
              href={isFromInventory ? "/inventory" : "/collection"}
              className="rounded-lg border border-border px-5 py-2.5 text-sm font-semibold text-muted-foreground hover:bg-accent hover:text-foreground transition-colors"
            >
              ← Back to {isFromInventory ? "Inventory" : "Collections"}
            </Link>
            <button
              type="button"
              onClick={() => setIsNegotiationDrawerOpen(true)}
