...
export default function DashboardPage() {
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-border/60 bg-card/60 p-5 backdrop-blur-md shadow-xs">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-base font-semibold tracking-tight text-slate-900 dark:text-slate-200">
            Recent Activity
          </h2>
        </div>
        ...
        <div className="space-y-3">
          {Array.from({ length: 3 }).map((_, i) => (
            <div
              key={i}
              className="h-12 w-full rounded-lg border border-border/40 bg-muted/30 animate-pulse"
            />
          ))}
        </div>

        {/* Footer info note */}
        <div className="mt-10 text-center text-xs font-normal text-muted-foreground/80">
